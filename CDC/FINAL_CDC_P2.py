...
def normalize_merged_data(myData):
	...
	attributes_to_normalize = ['YEAR', 'AVG_REL_EST_TOTAL', 'AGE_ADJUSTED_CANCER_RATE', 'Z_SCORE_AVG_REL_EST_TOTAL', 'Z_SCORE_AGE_ADJUSTED_CANCER_RATE']
...
def get_category_of_cancer_rate(myData):
	cols = ['AVG_REL_EST_TOTAL', 'AGE_ADJUSTED_CANCER_RATE']
	...
	chem_list = z_scores['AVG_REL_EST_TOTAL'].tolist()
	...
	for i in cancer_list:
		...
		if (i > 2.5):
			cancer_rate_level.append("very_high")
	...
	for i in chem_list:
		...
		if (i > 2.5):
			chem_rate_level.append("very_high")
...
def main():
	...
	merged_df = pd.read_csv('merged_data.csv', sep=',', encoding='latin1')
	...
	new_df = get_category_of_cancer_rate(merged_df)
	new_df = new_df.loc[:, new_df.columns.intersection(['YEAR', 'STATE_ABBR', 'AVG_REL_EST_TOTAL', 'AGE_ADJUSTED_CANCER_RATE', 'Z_SCORE_AVG_REL_EST_TOTAL', 'Z_SCORE_AGE_ADJUSTED_CANCER_RATE', 'CHEM_RATE_LEVEL', 'CANCER_RATE_LEVEL'])]
	new_df = normalize_merged_data(new_df)

	predictive_models(new_df, 7)
	print("end main")
